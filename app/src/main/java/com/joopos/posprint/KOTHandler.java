...
public class KOTHandler {
   ...
   /* public void handleKOT() {
        try {
            ...
            // ---------------------------------------------------------------
            // STEP 1: COUNT DUPLICATES PER PRINTER ID
            // ---------------------------------------------------------------
            Map<Integer, Integer> printerDuplicateCount = new HashMap<>();

            for (Iterator<String> it = details.keys(); it.hasNext(); ) {
                JSONObject obj = details.getJSONObject(it.next());

                Object printerObj = obj.get("printer");
                int printerId = -1;

                if (printerObj instanceof JSONArray) {
                    printerId = ((JSONArray) printerObj).optInt(0, -1);
                } else if (printerObj instanceof Number) {
                    printerId = ((Number) printerObj).intValue();
                }

                if (printerId != -1) {
                    printerDuplicateCount.put(
                            printerId,
                            printerDuplicateCount.getOrDefault(printerId, 0) + 1
                    );
                }
            }

            // ---------------------------------------------------------------
            // STEP 2: PRINT (DUPLICATE BASED ON PRINTER ID)
            // ---------------------------------------------------------------
            for (Iterator<String> keyIterator = details.keys(); keyIterator.hasNext(); ) {
                ...
                if (printerDetails == null) {
                    Log.e("TAG", "Printer not found for id=" + printerId);
                    continue;
                }

                String ip = printerDetails.optString("ip");
                int port = printerDetails.optInt("port", 9100);

                // ------------------------------
                // BUILD KOT TEXT (NO CHANGE)
                // ------------------------------
                String textToPrint = formatKOTText(response, objectDetails);

                // ------------------------------
                // DUPLICATE COUNT BASED ON PRINTER ID
                // ------------------------------
                int duplicateTimes =
                        printerDuplicateCount.getOrDefault(printerId, 1);

                PrintConnection printer = new PrintConnection(context);

                for (int i = 0; i < duplicateTimes; i++) {
                    int copyNo = i + 1;
                    printer.printWithStatusCheck(ip, port, textToPrint,
                            (success, msg) ->
