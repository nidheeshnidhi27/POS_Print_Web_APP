    ...
    public void printDrawerAndText(byte[] drawerPulse, String textToPrint, Callback callback) {
        executor.execute(() -> {
            ...
            try {
                ...
                int r0 = conn.bulkTransfer(outEp, drawerPulse, drawerPulse.length, 500);
                if (r0 < 0) throw new RuntimeException("drawer pulse failed");
                Charset cs = Charset.forName("CP858");
                byte[] data = textToPrint.getBytes(cs);
                byte[] lf = new byte[]{0x0A};
                byte[] feed = new byte[]{0x1B, 0x64, 0x02};
                ...
                success = true;
                message = "USB drawer open and printed";
            } catch (Exception e) {
                message = "USB drawer/text failed: " + e.getMessage();
                showNotification(message);
                Log.e(TAG, message, e);
            } finally {
                try { if (conn != null) conn.close(); } catch (Exception ignored) {}
